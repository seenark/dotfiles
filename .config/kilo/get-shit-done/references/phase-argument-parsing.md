...
```bash
PHASE_INFO=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
