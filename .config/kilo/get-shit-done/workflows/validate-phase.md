...
AGENT_SKILLS_AUDITOR=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-nyquist-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" config-get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read $HOME/.config/kilo/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `$HOME/.config/kilo/get-shit-done/templates/VALIDATION.md`
...
node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
