...
- **Global scope**: `$HOME/.config/kilo/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `$HOME/.config/kilo/notes/`)
...
2. Glob `$HOME/.config/kilo/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `$HOME/.config/kilo/notes/` — works in any directory
