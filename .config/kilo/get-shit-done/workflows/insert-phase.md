...
```bash
INIT=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
