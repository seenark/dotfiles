...
<required_reading>
@$HOME/.config/kilo/get-shit-done/references/verification-patterns.md
@$HOME/.config/kilo/get-shit-done/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.config/kilo/get-shit-done/bin/gsd-tools.cjs" verify key-links "$plan")
...
See $HOME/.config/kilo/get-shit-done/templates/verification-report.md for complete template.
</step>
